...
- `18_workflow_markdown_python/`: Markdown workflow with inline `python` fenced blocks
- `19_workflow_markdown_bash/`: Markdown workflow with an inline `bash` step, then LLM + inline `python` save
- `20_workflow_markdown_llm_first/`: Markdown workflow whose first step is LLM; prompt `{{ key }}` uses dict keys from `run(initial_input=...)` (equivalent to `{{ step_input.key }}`)
